...
public:
  ...
  void Update(const CUpdateList& i_list) {
    ...
    for (CUpdateList::size_type i=0; i<i_list.size(); ++i) {
      // Copy first member
      transform(i_list[i].first->begin(),
                i_list[i].first->end(), 
  ...
  virtual double GetValue(const CVectorDblPtr& i_ptrState) {
    ...
    return static_cast<double>(scaleOut(*fann_run(m_pFann, &i_ptrState->operator[](0))));
  }
...
double CNeuronalNetwork::GetValue(CVectorAnyPtr i_ptrState) {
  if (i_ptrState->empty()) {
    return 0.0;
  }
